...
const CanMsg HONDA_RADARLESS_TX_MSGS[] = {{0xE4, 0, 5}, {0x1C9, 0, 8}, {0x296, 2, 4}, {0x33D, 0, 8}};  // Bosch radarless
...
static int honda_bosch_fwd_hook(int bus_num, CANPacket_t *to_fwd) {
  ...
  if (bus_num == 2)  {
    int addr = GET_ADDR(to_fwd);
    int is_lkas_msg = (addr == 0xE4) || (addr == 0xE5) || (addr == 0x33D) || (addr == 0x33DA) || (addr == 0x33DB)|| (addr == 0x1C9);
    if (!is_lkas_msg) {
      bus_fwd = 0;
    }
  }
